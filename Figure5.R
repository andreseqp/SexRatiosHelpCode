...
for(i in 1:4){
  lines(diffz5(Bplot,param2,z1=0.5,z3=0.5,z5=0.5,omegRange,h,phirange[i])~
          omegRange,col=colors1[i],lty=lty1[1],lwd=2)
}
par(xpd=T,las=0)
text(-0.28,-0.05,expression(paste("Selection on ",italic(z[5]))),srt=90,cex=1.2)
...
for(j in 1:4){
  lines(diffz5(Bplot,param2,z1=out[1001,2],z3=out[1001,3],z5=out[1001,3],
...
for(i in 1:3){
  lines(diffz3(Bplot,param2,z1=0.5,z3=0.5,z5=z5Range,omegaPlot,hrange[i])~
          z5Range,col=colors1[i],lty=lty1[1],lwd=2)
}

par(xpd=T,las=0)
text(1.2,0.0,expression(paste("Selection on ",italic(z[3]))),srt=90,cex=1.2)
parameters <- c(F1=5,Sm=1,Sf=1,b=0,K=0.002,phi=0)
state <- c(z1=0.5,z3=0.5,z5=0.5,h=0,omega=0)
# Create output
times = seq(0,10000,by=10)
...
for(j in 1:3){
  lines(diffz3(Bplot,param2,z1=out[1001,2],z3=out[1001,3],z5=z5Range,omegaPlot,
               hrange[j])~z5Range,col=colors1[j],lty=lty1[2],lwd=2)
}
