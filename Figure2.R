...
dist<-read.table(list.dist[seed])
names(dist) <- c("z1","z2","z3","h","om")
...
#  SD
# polygon(x=c(Xs$Gen,Xs$Gen[length(Xs$Gen):1]),y=c(Xs$z2+Xs$sdz2,Xs$z2[length(Xs$z2):1]-Xs$sdz2[length(Xs$sdz2):1]),
#         col=colorPol[2],border=NA)
# polygon(x=c(Xs$Gen,Xs$Gen[length(Xs$Gen):1]),y=c(Xs$z3+Xs$sdz3,Xs$z3[length(Xs$z1):1]-Xs$sdz3[length(Xs$sdz3):1]),
#         col=colorPol[3],border=NA)
...
hist(dist$z3,breaks=30,col=colorPol[3],main="",ylab="",xlab="",axes=F)
polygon(x=c(-0.05,1.05,1.05,-0.05,-0.05),y=c(-10,-10,1000,1000,-10), col = "white",border=T)
par(plt=posPlot(numplotx=6,numploty=6,idplotx=6,idploty=2)-0.01,new=T)
hist(dist$z3,breaks=30,col=colorPol[3],main="",ylab="",xlab="",axes=F)
text(x=0.75,y=700,expression(z[3]),cex=2)
